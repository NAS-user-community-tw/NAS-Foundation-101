...
NAS 基礎觀念 合集
 ...
  - [領域知識 結構與複雜度 , 與 指引/領航支援系統](#domain-knowledge-complexity-and-guidance)
  )  
  - [介紹](#introduction)
  ...
  - [Books](#books)
  - [Paper](#paper)  
  - [Markdown 編輯器與語法參考](#markdown-reference)
...
# Domain Knowledge Complexity and Guidance
## 談 領域知識 結構與複雜度 , 與 指引/領航支援系統  

領域知識範圍內 , 包含了極大量的 術語 觀念 模型 架構 , 術語 以及 觀念 , 可以透過拉長學習時間 , 慢慢獲取相關學習成果 , 模型與架構 等較為有效與實用的成果 , 就不是閱讀幾篇文章可以得到的 . NAS 原廠希望透過 "易學易用" 的包裝 , 獲致行銷上的銷售成效 , 但是背後的 技術複雜程度 , 隨著 NAS 系統研發人員與投入時間及資源的增加 , 也只能沒有上限地繼續往上衝 , 從學習觀點來說 , "易學易用" 是 一種操控複雜度的降低 , 而 應用伺服器的本質 , 讓學習背景知識的複雜度 , 有增無減 , 這部分的進一步申論敘述 , 以及圖像示意 , 可以參考這一篇文章 : [從 旅客需要老司機 , 談 領域知識 結構與複雜度 , 與 指引 / 領航支援系統](/Domain_Knowledge_Complexity.md)  
...
### `常見的資安觀念誤區 : "我有了 磁碟陣列 (或者 NAS) 為什麼資料還是會消失 ?"`  

> 資訊安全 並不是 買二台 NAS 或者 買一台防火牆 這麼單純 . 更別說只有一台 NAS (或者沒有 NAS 或者任何檔案[備份](https://zh.wikipedia.org/wiki/%E5%A4%87%E4%BB%BD)機制的情況).

" 啊 , 我不是買了 防火牆 (NAS / 硬碟陣列 ...) , 怎麼還是出事了 (被駭 / 中毒 / 遇上勒索病毒 / 資料檔案全毀 / 硬碟故障 ...) ? "

其實是這樣的 , 就譬如 夫妻生了小孩 , 孩子生了之後 , 還得教養 , 否則沒人照顧 , 小孩子是很難自己孤單地健康成長的 .

以 防火牆 為例 , 它是規則導向 , 資安政策制定者 , 要能夠想清楚 , 企業組織內的 網路流量 的 來源與終點 , 核准通行的項目 / 規則 , 再來委派 防火牆的管理者 或者 委外廠商 , 協助將 防火牆 依照規劃去執行 管制 網路各區之間的流通 .

NAS 也是如此 , 多槽 NAS 裡面依靠的是 磁碟陣列 , 陣列能夠提供是 : 大一點的總容量 / 可能高一點的總傳輸性能 / 陣列本身的容錯 , 但是 單純的 NAS 與 陣列 , 完全不是備份的同義詞 , 企業組織在使用的時候 , 得要規劃出 人員與檔案間的存取關聯性 , 考慮到 各種檔案遺失風險 , 再來 利用手邊的 儲存與管理資源 , 執行 風險控管的任務 .

把這些 防患未然的事情都想清楚想透徹了 , 規劃與執行出來的實際 架構與設施 , 才能讓人日後遇到狀況沒這麼緊張 , 因為都在預料內 , 早就做好準備 .

### 善用 NAS , 減少檔案遺損的憾事

NAS 裡面因為有 作業系統 / 伺服器套件 / 使用者端軟體 等等的組合協助之下 , 因此比起一般 人工手動備份 , 多了許多機制以及選項 . 無論 個人 / 家庭 / 企業組織 , 都應該預先考慮 [備份 3-2-1 法則](https://www.ithome.com.tw/tech/101369) . 實務上來說 , 一開始可能沒有辦法完全做到 , 後續要盡量往這個目標前進 , 依據個別的 預算編列 / 環境條件 , 保持不只一份資料實體的狀態 , 這樣才能防止 單一資料受損 , 甚至遺失的痛苦遭遇 . 
社會大眾開始重視多版本檔案備份 , 大概是從 [勒索病毒](https://zh.wikipedia.org/wiki/%E5%8B%92%E7%B4%A2%E8%BB%9F%E9%AB%94) 猖獗發做的那段期間開始 , 勒索病毒能夠在電腦 乃至於內部網路內 , 尋找受害者 (無論是 硬碟內的 或者 網路芳鄰內的檔案) , 大規模卯起來進行加密 , 導致使用者忽然發現 大量的工作檔案 或者 珍貴文件 , 完全無法開啟 , 接著就會收到 勒索者的訊息 , 告訴苦主 , 必須在某個期限內 , 支付特定數量的[比特幣](https://buzzorange.com/techorange/2017/05/16/wanna-cry-why-bitcoin/) (因為難以追蹤) , 才會提供解密的程序與解密金鑰 .
...
另外寫成一篇 [文章](/Technical_Services.md) , 請讀者們參考 .  
